...
def get_current_mac(answers=None, interface=None):
    ...
    try:
        ifconfig_res = subprocess.check_output(
            ["ifconfig", interface], stderr=NULL_DEVICE)
        # decode what is a bytes-like object
        decoded_res = ifconfig_res.decode("utf-8")
        current_mac_res = re.search(
            r"\w\w:\w\w:\w\w:\w\w:\w\w:\w\w", decoded_res)
        # does the device even have a MAC address?
        if (current_mac_res is not None):
...
def validate_new_mac_persistence(interface, new_mac):
    ...
    if (current_mac == new_mac):
        print(
            f"[+] Successfully updated device {interface} MAC address to {new_mac}.")
    else:
        print(
            f"[-] Failed to update/persist to specified MAC address {new_mac} for device {interface}")
...
def validate_new_mac_format(answers=None, new_mac=None):
    ...
    return True if valid_mac_res is not None else False
