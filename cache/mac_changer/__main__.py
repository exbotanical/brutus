...
import inquirer
from .mac_changer import get_current_mac, validate_new_mac_format, generate_mac, change_mac_address, validate_new_mac_persistence
...
def main():
    ...
    try:
        questions = [
            inquirer.Text(
                "interface",
                message="Enter the name of a wireless interface for cloaking",
                validate=get_current_mac),
            inquirer.List(
                "generation_status",
            ...
            inquirer.Text(
                "provided_mac",
                message="Enter new MAC address",
                validate=validate_new_mac_format,
                ignore=auto_mode),
            inquirer.List(
                "oui_status",
                "Enforce a specific OUI?",
                choices=[
                    "yes",
                    "no"],
                ignore=manual_mode),
        ...
        if (new_mac is not None):
            print(
                f"[+] Current MAC for device {interface}: {str(get_current_mac(None, interface))}\n")
